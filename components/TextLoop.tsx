...
const buildPath = (shape: TextLoopShape, curviness: number, ribbonWidth: number, viewH: number): string => {
  ...
  switch (shape) {
    ...
    case 'infinity': {
      const r = 400 + c * 1.4;
      const h = Math.min(140 + c * 0.95, room);
      return [
        `M ${CX} ${CY}`,
...
const TextLoop = ({
  ...
  const viewH = useMemo(() => {
    if (shape === 'wave') {
      const a = Math.max(0, curviness) * 2.2;
      return Math.max(130, Math.round(ribbonWidth + a * 2 + EDGE_PAD * 4));
    }
    if (shape === 'line') {
      return Math.max(100, Math.round(ribbonWidth + EDGE_PAD * 4));
    }
    return 520;
